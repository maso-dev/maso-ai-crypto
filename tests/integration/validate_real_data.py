    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            results["mvp_status"] = {
                ...
                "livecoinwatch_status": mvp_status.get("components", {})
                .get("livecoinwatch", {})
                .get("status"),
                "news_cache_status": mvp_status.get("components", {})
                .get("news_cache", {})
                .get("status"),
                "hybrid_rag_status": mvp_status.get("components", {})
                .get("hybrid_rag", {})
                .get("status"),
                "graph_mock_mode": mvp_status.get("components", {})
                .get("hybrid_rag", {})
                .get("graph_mock_mode"),
            }
            print(f"   ✅ System Health: {mvp_status.get('system_health')}")
            print(
                f"   ✅ LiveCoinWatch: {mvp_status.get('components', {}).get('livecoinwatch', {}).get('status')}"
            )
            print(
                f"   ✅ News Cache: {mvp_status.get('components', {}).get('news_cache', {}).get('status')}"
            )
            print(
                f"   ⚠️  Graph RAG Mock Mode: {mvp_status.get('components', {}).get('hybrid_rag', {}).get('graph_mock_mode')}"
            )
        except Exception as e:
        ...
        try:
            ...
            print(
                f"   📊 Has Indicators: {bool(tech_analysis.get('technical_indicators'))}"
            )
...
def print_summary(results):
    ...
    if tech.get("current_price", 0) > 0:
        print(
            f"📊 Technical Analysis: ✅ REAL DATA (BTC: ${tech.get('current_price', 0):,.2f})"
        )
    else:
