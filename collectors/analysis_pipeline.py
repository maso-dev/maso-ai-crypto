...
try:
    ...
    Neo4jGraphRAG = None
from utils.temporal_context import enhance_article_with_temporal_context, sort_articles_by_temporal_relevance
...
class AnalysisPipeline:
    ...
    def __init__(self, raw_db_path: str = "data/raw_news.db"):
        ...
        self.embedding_client = OptimizedEmbeddingClient() if OptimizedEmbeddingClient else None
    ...
    async def enrich_article(self, article: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            enriched = {
                ...
                'time_relevance': getattr(enrichment_result, 'time_relevance', 'recent'),
    ...
    async def create_embeddings(self, enriched_article: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Create embeddings for the enriched article."""
        try:
            # Prepare text for embedding
            embedding_text = f"{enriched_article['title']}\n\n{enriched_article['content'] or ''}"
            ...
            logger.error(f"Error creating embedding for article {enriched_article['id']}: {e}")
            return enriched_article
    ...
    async def store_in_vector_db(self, enriched_article: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Error storing article {enriched_article['id']} in vector DB: {e}")
            return False
    ...
    async def store_in_graph_db(self, enriched_article: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Error storing article {enriched_article['id']} in graph DB: {e}")
            return False
    ...
    async def process_article_batch(self, articles: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    async def run_analysis_cycle(self, batch_size: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            while True:
                ...
                logger.info(f"Batch completed: {batch_results['processed']}/{len(articles)} processed")
        ...
        logger.info(f"✅ Analysis cycle completed in {cycle_results['duration_seconds']:.1f}s")
