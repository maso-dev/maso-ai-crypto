...
class TemporalScheduler:
    ...
    async def run_full_cycle(self, hours_back: int = 24, batch_size: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            collection_results = await self.news_ingestor.run_collection_cycle(hours_back)
            ...
            analysis_results = await self.analysis_pipeline.run_analysis_cycle(batch_size)
        ...
        logger.info(f"✅ Full cycle completed in {full_results['total_duration_seconds']:.1f}s")
        
        return full_results
    
    def get_recent_logs(self, run_type: Optional[str] = None, limit: int = 10) -> List[Dict[str, Any]]:
        """Get recent run logs."""
        try:
            log_files = []
    ...
    def print_status_summary(self) -> None:
        ...
        if recent_full:
            ...
            for i, log in enumerate(recent_full[:3]):
                ...
                collection_articles = log.get('collection_results', {}).get('total_new_articles', 0)
                processed_articles = log.get('analysis_results', {}).get('total_processed', 0)
                
                print(f"   {i+1}. {status} {log.get('start_time', 'Unknown')} "
        ...
        if stats:
            ...
            unprocessed = stats.get('processing_status', {}).get(0, 0)  # 0 = False = unprocessed
            processed = stats.get('processing_status', {}).get(1, 0)    # 1 = True = processed
...
async def main():
    ...
    parser.add_argument('--mode', choices=['collect', 'analyze', 'full', 'status'], 
                       default='full', help='Operation mode')
    parser.add_argument('--hours-back', type=int, default=24, 
                       help='Hours back to collect news (default: 24)')
    parser.add_argument('--batch-size', type=int, default=50, 
                       help='Analysis batch size (default: 50)')
