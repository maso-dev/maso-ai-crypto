...
async def test_qdrant_step_by_step():
    ...
    try:
        ...
        if connection_status['status'] == 'connected':
            ...
            print(f"❌ Qdrant connection failed: {connection_status.get('error', 'unknown error')}")
            return False
    ...
    try:
        ...
        # Test document
        test_content = "Bitcoin reaches new all-time high as institutional adoption increases"
        test_metadata = {
            "symbols": ["BTC"],
...
async def test_with_fallback():
    ...
    try:
        ...
        test_content = "Ethereum 2.0 staking reaches new milestones as DeFi continues to grow"
        test_metadata = {
            "symbols": ["ETH"],
            "category": "crypto_news",
            "sentiment": "positive",
