    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'text/html' in response.headers.get('content-type', ''):
                ...
                print(f"   ❌ Root endpoint returns wrong content type: {response.headers.get('content-type')}")
                return False
        else:
...
def test_replit_health_endpoint():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"   ✅ Replit health endpoint working: {data.get('status', 'unknown')}")
            return True
        else:
...
def check_environment_variables():
    ...
    optional_vars = [
        "NEWS_API_KEY", "TAVILY_API_KEY", "BINANCE_API_KEY",
        "MILVUS_TOKEN", "NEO4J_URI", "QDRANT_URL"
    ]
