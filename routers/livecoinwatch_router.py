...
async def trigger_price_collection(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error triggering collection: {str(e)}")
...
async def trigger_historical_collection(
    ...
    try:
        background_tasks.add_task(collect_historical_data, data_request.symbol, data_request.days)
        ...
        raise HTTPException(status_code=500, detail=f"Error triggering historical collection: {str(e)}")
...
async def trigger_indicators_calculation(
    ...
    try:
        background_tasks.add_task(calculate_technical_indicators, data_request.symbol, data_request.days)
        ...
        raise HTTPException(status_code=500, detail=f"Error triggering indicators calculation: {str(e)}")
...
async def get_latest_price_data(symbols: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving price data: {str(e)}")
