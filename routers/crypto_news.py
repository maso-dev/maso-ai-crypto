...
async def populate_crypto_news_rag(request: PopulateNewsRequest) -> PopulateNewsResponse:
    ...
    try:
        ...
        articles = await fetch_news_articles(terms=["cryptocurrency", "bitcoin", "ethereum"], hours_back=request.hours_back)
        ...
        if pipeline_result["vector_data"]:
            print(f"🗄️ Inserting {len(pipeline_result['vector_data'])} chunks into Milvus...")
        ...
        raise HTTPException(status_code=500, detail=f"Error populating news RAG: {str(e)}")
...
async def get_trending_topics() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving trending topics: {str(e)}")
