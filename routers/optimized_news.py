...
class OptimizedNewsService:
    ...
    def get_market_insights(self, articles: List[Dict[str, Any]]) -> MarketInsights:
        ...
        # Urgency statistics
        urgency_scores = [a.get('urgency_score', 0) for a in articles if a.get('urgency_score')]
        urgency_stats = {
            'average': sum(urgency_scores) / len(urgency_scores) if urgency_scores else 0,
...
async def get_optimized_news(
    crypto_symbols: Optional[str] = Query(None, description="Comma-separated crypto symbols"),
    ...
    symbols_list = None
    if crypto_symbols:
        symbols_list = [s.strip().upper() for s in crypto_symbols.split(",") if s.strip()]
    ...
    article_models = []
    for article in articles:
        article_models.append(NewsArticle(
            id=article['id'],
            title=article['title'],
    ...
    try:
        with sqlite3.connect(news_service.raw_db_path) as conn:
            ...
            return {
                ...
                "processing_percentage": round((processed_articles / total_articles * 100), 1) if total_articles > 0 else 0,
