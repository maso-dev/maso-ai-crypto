...
from utils.intelligent_news_cache import get_portfolio_news, get_cache_statistics, clear_expired_cache
...
async def get_comprehensive_portfolio_context(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting portfolio context: {str(e)}")
...
async def get_portfolio_insights() -> List[PortfolioInsightResponse]:
    ...
    try:
        ...
        return [PortfolioInsightResponse(**insight) for insight in context_data["portfolio_insights"]]
...
async def get_trading_opportunities() -> List[TradingOpportunityResponse]:
    ...
    try:
        ...
        return [TradingOpportunityResponse(**opp) for opp in context_data["trading_opportunities"]]
...
async def get_symbol_context(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting symbol context: {str(e)}")
...
async def get_symbol_analysis(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting symbol analysis: {str(e)}")
...
async def get_symbol_news(symbol: str, hours_back: int = 24) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting symbol news: {str(e)}")
...
async def get_portfolio_news(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting portfolio news: {str(e)}")
...
async def get_context_info() -> Dict[str, Any]:
    ...
    return {
        ...
        "supported_symbols": [
            "BTC", "ETH", "XRP", "SOL", "DOGE", "AVAX", "ADA", "DOT", "LINK", "MATIC"
        ]
    } 
