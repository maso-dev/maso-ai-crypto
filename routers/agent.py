...
class AgentAnalysisRequest(BaseModel):
    ...
    symbols: Optional[List[str]] = Field(
        default=None, description="Specific symbols to analyze"
    )
    include_news: bool = Field(
        default=True, description="Include market news in analysis"
    )
    risk_tolerance: str = Field(
        default="MEDIUM", description="User risk tolerance: LOW, MEDIUM, HIGH"
    )
...
async def get_agent_analysis(request: AgentAnalysisRequest) -> AgentAnalysisResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating agent analysis: {str(e)}"
        )


@router.get("/recommendations", response_model=List[Dict[str, Any]])
async def get_recommendations(
    limit: int = 5, risk_level: Optional[str] = None, action_type: Optional[str] = None
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if risk_level:
            recommendations = [
                r for r in recommendations if r.risk_level.value == risk_level.upper()
            ]

        if action_type:
            recommendations = [
                r for r in recommendations if r.action_type.value == action_type.upper()
            ]
...
async def get_agent_insights() -> AgentInsightsResponse:
    ...
    try:
        ...
        risk_metrics = {
            ...
            "high_risk_recommendations": len(
                [r for r in analysis.recommendations if r.risk_level == RiskLevel.HIGH]
            ),
        }
        ...
        for rec in analysis.recommendations:
            if rec.confidence_score > 0.8:
                opportunities.append(
                    f"{rec.action_type.value} {rec.asset}: {rec.reason}"
                )
            elif rec.risk_level == RiskLevel.HIGH:
                warnings.append(
                    f"High-risk action: {rec.action_type.value} {rec.asset}"
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting agent insights: {str(e)}"
        )
...
async def execute_recommendation(action: RecommendationAction) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error executing recommendation: {str(e)}"
        )
...
async def get_market_sentiment() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting market sentiment: {str(e)}"
        )
...
def _generate_agent_insights(analysis: AgentAnalysis) -> str:
    ...
    if analysis.portfolio_analysis.total_roi_percentage > 10:
        insights.append(
            f"Your portfolio is performing well with {analysis.portfolio_analysis.total_roi_percentage:.1f}% ROI"
        )
    elif analysis.portfolio_analysis.total_roi_percentage < -5:
        insights.append(
            f"Portfolio is down {abs(analysis.portfolio_analysis.total_roi_percentage):.1f}% - consider defensive positions"
        )
    ...
    if regime == "BULL":
        insights.append(
            "Market is in bullish territory - good time for growth positions"
        )
    elif regime == "BEAR":
    ...
    high_confidence_recs = [
        r for r in analysis.recommendations if r.confidence_score > 0.8
    ]
    if high_confidence_recs:
        insights.append(
            f"Found {len(high_confidence_recs)} high-confidence opportunities"
        )

    return (
        ". ".join(insights)
        if insights
        else "Portfolio is stable with no immediate action required."
    )
...
def _extract_positive_factors(analysis: AgentAnalysis) -> List[str]:
    ...
    if analysis.portfolio_analysis.total_roi_percentage > 0:
        positives.append(
            f"Positive ROI: {analysis.portfolio_analysis.total_roi_percentage:.1f}%"
        )
