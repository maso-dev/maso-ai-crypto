...
class MarketSummaryRequest(BaseModel):
    ...
    always_include_base_coins: bool = Field(default=True, description="Always include BTC and ETH in analysis")
...
async def portfolio_market_summary(request: MarketSummaryRequest) -> MarketSummaryResponse:
    ...
    try:
        ...
        summary, recommendations = await get_market_summary(limited_news, symbols_to_analyze)
        
        # Convert recommendations string to list
        recommendations_list = [rec.strip() for rec in recommendations.split('\n') if rec.strip()] if recommendations else []
...
async def get_etf_comparison() -> ETFComparisonResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving ETF comparison: {str(e)}")
