...
async def test_hybrid_rag_system():
    ...
    try:
        ...
        for i, result in enumerate(results[:2]):
            print(f"   {i+1}. {result.title[:50]}... (confidence: {result.confidence_score:.2f})")
...
async def test_hybrid_statistics():
    ...
    try:
        ...
        print(f"   Graph RAG: {'✅' if stats.get('graph_rag', {}).get('connected', False) else '❌'}")
        print(f"   Hybrid System: {stats.get('hybrid_system', {}).get('status', 'unknown')}")
    except Exception as e:
...
async def test_api_endpoints():
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"✅ Query types endpoint: {len(data.get('query_types', []))} types")
            else:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"✅ Hybrid search endpoint: {data.get('results_count', 0)} results")
            else:
