...
async def test_enhanced_pipeline_with_quality():
    ...
    try:
        ...
        result = await pipeline.process_crypto_news(
            symbols=symbols, hours_back=24, enable_enrichment=True, max_articles=20
        )
        ...
        if result["success"]:
            ...
            print(
                f"   Symbols searched: {', '.join(processing_info['symbols_searched'])}"
            )
            print(f"   Time range: {processing_info['hours_back']} hours back")
            print(f"   Enrichment enabled: {processing_info['enrichment_enabled']}")
            print(
                f"   Quality filtering enabled: {processing_info['quality_filtering_enabled']}"
            )
            ...
            if quality_metadata.get("quality_filtering_enabled"):
                print(f"\n🔍 Quality Filtering Results:")
                print(
                    f"   Articles before filtering: {quality_metadata['articles_before_filtering']}"
                )
                print(
                    f"   Articles after filtering: {quality_metadata['articles_after_filtering']}"
                )
            ...
            if metadata:
                ...
                if enrichment_stats:
                    print(
                        f"   Enriched articles: {enrichment_stats.get('enriched_count', 0)}"
                    )
                    print(
                        f"   Average sentiment: {enrichment_stats.get('avg_sentiment', 0):.2f}"
                    )
                    print(
                        f"   Average trust: {enrichment_stats.get('avg_trust', 0):.2f}"
                    )
                    print(
                        f"   Top categories: {', '.join(enrichment_stats.get('top_categories', [])[:3])}"
                    )
...
async def test_convenience_function():
    ...
    try:
        result = await get_enhanced_crypto_news(
            symbols=["BTC", "ETH"], hours_back=12, enable_enrichment=True
        )
        ...
        if result["success"]:
            ...
            print(
                f"   Quality filtering: {result['processing_info']['quality_filtering_enabled']}"
            )
        else:
