...
async def test_refresh_processor():
    ...
    try:
        ...
        print(f"⏱️ Duration: {result.processing_metadata.get('duration_seconds', 0):.2f} seconds")
    ...
    try:
        ...
        print(f"⏱️ Duration: {result.processing_metadata.get('duration_seconds', 0):.2f} seconds")
        print(f"💰 Price Symbols: {result.processing_metadata.get('price_symbols', 0)}")
        print(f"📊 Indicators: {result.processing_metadata.get('indicators_calculated', 0)}")
    ...
    try:
        ...
        if data_collected:
            print(f"📰 News Articles: {result.processing_metadata.get('news_articles', 0)}")
            print(f"💰 Price Symbols: {result.processing_metadata.get('price_symbols', 0)}")
            print(f"📊 Indicators: {result.processing_metadata.get('indicators_calculated', 0)}")
            print(f"🔍 Filtered Articles: {result.processing_metadata.get('filtered_articles', 0)}")
            print(f"🧠 AI Processing: {result.processing_metadata.get('ai_processing', 'not completed')}")
            print(f"🔗 RAG Updated: {result.processing_metadata.get('rag_updated', False)}")
