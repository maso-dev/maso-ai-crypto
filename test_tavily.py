...
async def test_tavily_integration():
    ...
    try:
        response = await search_tavily_news("Bitcoin cryptocurrency news", max_results=5)
        
        print(f"✅ News Search: {'SUCCESS' if response.total_results > 0 else 'NO RESULTS'}")
    ...
    try:
        response = await search_tavily_finance("Bitcoin current price market cap", max_results=3)
        
        print(f"✅ Finance Search: {'SUCCESS' if response.total_results > 0 else 'NO RESULTS'}")
        print(f"📊 Total Results: {response.total_results}")
        print(f"⏱️ Search Time: {response.search_time:.2f}s")
        print(f"🔍 Query: {response.query}")
    ...
    try:
        response = await tavily_client.search_web("cryptocurrency blockchain technology", max_results=3)
        
        print(f"✅ Web Search: {'SUCCESS' if response.total_results > 0 else 'NO RESULTS'}")
        print(f"📊 Total Results: {response.total_results}")
        print(f"⏱️ Search Time: {response.search_time:.2f}s")
        print(f"🔍 Query: {response.query}")
