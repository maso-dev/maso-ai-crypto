...
async def test_livecoinwatch():
    ...
    try:
        ...
        for data in price_data:
            print(
                f"   {data.symbol}: ${data.price_usd:,.2f} (24h: {data.change_24h:+.2f}%)"
            )
    ...
    try:
        ...
        if historical_data:
            ...
            print(
                f"   Latest BTC: ${latest.close_price:,.2f} (Volume: ${latest.volume:,.0f})"
            )
    ...
    try:
        ...
        if indicators:
            ...
            if bollinger:
                print(
                    f"   Bollinger Bands: ${bollinger.get('lower', 0):,.0f} - ${bollinger.get('upper', 0):,.0f}"
                )
    ...
    try:
        ...
        for symbol, price_data in latest_prices.items():
            print(
                f"   {symbol}: ${price_data.price_usd:,.2f} (Market Cap: ${price_data.market_cap:,.0f})"
            )
