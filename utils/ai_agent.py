...
class CryptoAIAgent:
            ...
            result = await chain.ainvoke(
                {
                    "task": state.task.value,
                    "query": state.query,
                    "market_analysis": state.analysis_results.get(
                        "market_analysis", ""
                    ),
    ...
    async def _validate_recommendations(
        ...
        try:
            ...
            if state.recommendations:
                avg_confidence = sum(
                    r.get("confidence", 0) for r in state.recommendations
            ...
            print(
                f"🎉 AI Agent: Task completed with confidence {state.confidence_score:.2f}"
            )
    ...
    async def execute_task(
        ...
        # Initialize state
        state = AgentState(task=task, query=query, symbols=symbols or [])
...
async def analyze_market_sentiment(
    symbols: List[str], config: Optional[RunnableConfig] = None
) -> AgentState:
...
async def generate_portfolio_recommendations(
    ...
    return await execute_agent_task(
        AgentTask.PORTFOLIO_RECOMMENDATION, query, symbols, config
    )
