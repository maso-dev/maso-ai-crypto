...
""")
    ...
    async def analyze_portfolio(self, portfolio_data: PortfolioData, market_news: List[Dict[str, Any]]) -> PortfolioAnalysis:
    ...
    async def calculate_confidence(self, recommendation: Recommendation, portfolio_data: PortfolioData) -> float:
        ...
        try:
            ...
            final_confidence = min(1.0, max(0.0, base_confidence + data_quality_bonus + risk_adjustment))
    ...
    async def generate_complete_analysis(
        ...
        market_news = await query_news_for_symbols(symbols) if symbols else []
            ...
            for rec in recommendations:
                rec.confidence_score = await self.calculate_confidence(rec, portfolio_data)
            
            # Generate market summary
            market_summary = self._generate_market_summary(portfolio_analysis, market_news)
            
            # Generate risk assessment
            risk_assessment = self._generate_risk_assessment(portfolio_analysis, recommendations)
    ...
    def _prepare_portfolio_summary(self, portfolio_data: PortfolioData) -> str:
        ...
        for asset in portfolio_data.assets:
            roi_text = f"ROI: {asset.roi_percentage:.2f}%" if asset.roi_percentage else "ROI: N/A"
            cost_basis_text = f"Cost: ${asset.cost_basis:,.2f}" if asset.cost_basis else "Cost: N/A"
    ...
    def _fallback_recommendations(self, portfolio_data: PortfolioData) -> List[Recommendation]:
    ...
    def _generate_market_summary(self, analysis: PortfolioAnalysis, news: List[Dict[str, Any]]) -> str:
    ...
    def _generate_risk_assessment(self, analysis: PortfolioAnalysis, recommendations: List[Recommendation]) -> str:
        """Generate risk assessment."""
        risk_summary = f"Overall portfolio risk: {analysis.portfolio_risk_score:.2f}/1.0. "
...
async def generate_agent_analysis(portfolio_data: PortfolioData, symbols: List[str] = None) -> AgentAnalysis:
