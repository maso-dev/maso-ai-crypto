...
class DataQualityFilter:
    ...
    def __init__(self):
        ...
        self.reliable_sources = {
            ...
            "coindesk.com", "cointelegraph.com", "bitcoin.com", "decrypt.co",
            "theblock.co", "cryptonews.com", "ambcrypto.com", "newsbtc.com",
            "cryptoslate.com", "bitcoinist.com", "livebitcoinnews.com"
        }
        
        # Known unreliable sources (blacklist)
        self.unreliable_sources = {
            "cryptodaily.co.uk", "cryptonewsz.com", "cryptopolitan.com",
            "cryptonews.com", "cryptoslate.com", "bitcoinist.com"
        }
    ...
    async def filter_articles(self, articles: List[Dict[str, Any]], 
                            symbols: Optional[List[str]] = None) -> List[FilteredArticle]:
        """
        ...
        logger.info(f"Quality filtering complete: {approved_count}/{len(articles)} articles approved")
        
        return filtered_articles
    
    async def _filter_single_article(self, article: Dict[str, Any], 
                                   symbols: Optional[List[str]] = None) -> FilteredArticle:
        ...
        rejection_reason = None if is_approved else self._get_rejection_reason(quality_metrics)
    ...
    async def _calculate_quality_metrics(self, title: str, description: str, 
                                       content: str, source: str, url: str,
                                       full_text: str, symbols: Optional[List[str]] = None) -> QualityMetrics:
        ...
        ai_analysis = await self._ai_quality_analysis(full_text, source) if self.openai_available else None
        
        # Combine metrics
        overall_score = self._calculate_overall_score(
            source_reliability, content_quality, clickbait_score, 
            relevance_score, verification_score, ai_analysis
        )
    ...
    def _check_source_reliability(self, source: str, url: str) -> float:
        ...
        # Check whitelist
        if domain in self.reliable_sources or source.lower() in [s.split('.')[0] for s in self.reliable_sources]:
            return 0.9
        
        # Check blacklist
        if domain in self.unreliable_sources or source.lower() in [s.split('.')[0] for s in self.unreliable_sources]:
            return 0.1
        
        # Check for common reliable patterns
        reliable_patterns = [
            r'reuters', r'bloomberg', r'cnbc', r'wsj', r'ft',
            r'coindesk', r'cointelegraph', r'bitcoin\.com', r'decrypt',
            r'theblock', r'cryptonews', r'ambcrypto', r'newsbtc'
        ]
        
        for pattern in reliable_patterns:
            if re.search(pattern, domain, re.IGNORECASE) or re.search(pattern, source, re.IGNORECASE):
                return 0.8
        
        # Check for suspicious patterns
        suspicious_patterns = [
            r'crypto.*daily', r'crypto.*news', r'bitcoin.*news',
            r'crypto.*slate', r'crypto.*ist', r'live.*bitcoin'
        ]
    ...
    def _analyze_content_quality(self, title: str, description: str, content: str) -> float:
        ...
        crypto_terms = [
            ...
            'nft', 'smart contract', 'mining', 'staking', 'yield',
            'liquidity', 'volatility', 'market cap', 'trading volume'
        ]
    ...
    def _calculate_verification_score(self, source_reliability: float, 
                                    content_quality: float, clickbait_score: float) -> float:
    ...
    async def _ai_quality_analysis(self, text: str, source: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if "quality_score" in ai_content:
                try:
                    quality_match = re.search(r'quality_score["\s:]+([0-9.]+)', ai_content)
                    if quality_match:
            ...
            if "reliability_score" in ai_content:
                try:
                    reliability_match = re.search(r'reliability_score["\s:]+([0-9.]+)', ai_content)
    ...
    def _calculate_overall_score(self, source_reliability: float, content_quality: float,
                               clickbait_score: float, relevance_score: float,
                               verification_score: float, ai_analysis: Optional[Dict[str, Any]] = None) -> float:
        ...
        if ai_analysis:
            ...
            overall_score = (base_score * 0.7) + ((ai_quality + ai_reliability) / 2 * 0.3)
        else:
    ...
    def _identify_issues_and_recommendations(self, source_reliability: float,
                                           content_quality: float, clickbait_score: float,
                                           relevance_score: float, verification_score: float,
                                           ai_analysis: Optional[Dict[str, Any]] = None) -> Tuple[List[str], List[str]]:
    ...
    def _evaluate_approval(self, quality_metrics: QualityMetrics) -> bool:
        ...
        return (
            ...
            quality_metrics.clickbait_score < self.clickbait_threshold and
            quality_metrics.relevance_score >= self.relevance_threshold
        )
...
async def filter_news_articles(articles: List[Dict[str, Any]], 
                             symbols: List[str] = None) -> List[FilteredArticle]:
    """Filter news articles for quality."""
    return await data_quality_filter.filter_articles(articles, symbols)

async def get_quality_metrics(article: Dict[str, Any], 
                            symbols: List[str] = None) -> QualityMetrics:
    """Get quality metrics for a single article."""
    filtered_article = await data_quality_filter._filter_single_article(article, symbols)
