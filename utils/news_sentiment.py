...
class NewsAnalyzer:
    ...
    async def get_crypto_news(
        ...
        try:
            ...
            # Fetch news articles
            articles_data = await fetch_news_articles(
                search_terms[:5]
            ...
            for article_data in articles_data:
                try:
                    ...
                    article = NewsArticle(
                        ...
                        breaking_news=self._is_breaking_news(
                            article_data.get("title", ""), published_at
                        ),
                    )
    ...
    async def analyze_market_sentiment(
        ...
        try:
            ...
            confidence_score = min(
                len(articles) / 20.0, 1.0
            )  # Max confidence with 20+ articles
    ...
    async def get_market_context(
        ...
        try:
            ...
            key_catalysts = [
                topic for topic in sentiment.key_topics if self._is_catalyst(topic)
            ]
            risk_factors = [
                topic for topic in sentiment.key_topics if self._is_risk_factor(topic)
            ]
            opportunities = [
                topic for topic in sentiment.key_topics if self._is_opportunity(topic)
            ]
    ...
    async def _analyze_article_sentiment(self, article: NewsArticle) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"score": score, "impact": impact, "topics": topics}
    ...
    def _is_breaking_news(self, title: str, published_at: datetime) -> bool:
        ...
        has_breaking_keywords = any(
            keyword in title.lower() for keyword in breaking_keywords
        )

        return is_recent and has_breaking_keywords

    def _determine_market_impact(
        self, articles: List[NewsArticle], avg_sentiment: float
    ) -> str:
