...
class Neo4jGraphRAG:
        ...
        try:
            with self.driver.session() as session:
                ...
                if article_data.get("sentiment_score"):
                    sentiment = "positive" if article_data["sentiment_score"] > 0.1 else "negative" if article_data["sentiment_score"] < -0.1 else "neutral"
                    session.run("""
                        MATCH (n:NewsArticle {id: $article_id})
    ...
    async def _search_related_articles(self, session, query: GraphQuery) -> List[Dict[str, Any]]:
    ...
    async def _search_entity_network(self, session, query: GraphQuery) -> List[Dict[str, Any]]:
    ...
    async def _search_sentiment_analysis(self, session, query: GraphQuery) -> List[Dict[str, Any]]:
    ...
    async def _search_trending_topics(self, session, query: GraphQuery) -> List[Dict[str, Any]]:
...
async def insert_news_to_graph(article_data: Dict[str, Any], entities: Optional[List[Dict[str, Any]]] = None) -> Optional[str]:
...
async def search_graph(query_type: str, parameters: Dict[str, Any], limit: int = 10) -> List[Dict[str, Any]]:
