...
class RealTimeDataManager:
                    ...
                    try:
                        ...
                        if 's' in data:  # Symbol
                            ...
                            price_update = CryptoPrice(
                                ...
                                market_cap=float(data['v']) * float(data['c']),  # Approximate
                                timestamp=datetime.now(timezone.utc),
                                source="binance"
                            )
    ...
    async def _livecoinwatch_api(self):
        ...
        symbols = ["BTC", "ETH", "ADA", "DOT", "LINK", "SOL", "XRP", "DOGE", "AVAX", "MATIC"]
        ...
        while self.running:
            try:
                                ...
                                if response.status == 200:
                                    ...
                                    if data and "rate" in data:
                                        price_update = CryptoPrice(
                                            ...
                                            change_24h=float(data.get("delta", {}).get("day", 0)) * 100,  # Convert to percentage
                                    ...
                                    print(f"⚠️ LiveCoinWatch API returned status {response.status} for {symbol}")
                        except Exception as e:
                ...
                print("⏰ LiveCoinWatch API request timed out, falling back to mock data")
                await self._mock_data_stream()
                break
            except Exception as e:
