...
class EnhancedNewsPipeline:
    ...
    async def process_crypto_news(
        ...
        try:
            ...
            print(
                f"   ✅ Quality filtering complete: {len(approved_articles)}/{len(articles)} articles approved"
            )
            print(
                f"   ❌ Rejected {len(rejected_articles)} articles for quality issues"
            )
        ...
        try:
            if filtered_articles is not None:
                quality_metadata = {
                    ...
                    "rejected_count": len(
                        [fa for fa in filtered_articles if not fa.is_approved]
                    ),
                }
            else:
                quality_metadata = {
            ...
            quality_metadata = {"quality_filtering_enabled": False, "error": str(e)}
        ...
        return {
            ...
            "processing_info": {
                ...
                "quality_filtering_enabled": quality_metadata.get(
                    "quality_filtering_enabled", False
                ),
    ...
    def _analyze_articles(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if any("enrichment" in art for art in articles):
            ...
            trusts = [
                art["enrichment"]["trust"] for art in articles if "enrichment" in art
            ]
            categories = []
            for art in articles:
                if "enrichment" in art:
...
async def get_enhanced_crypto_news(
    ...
    return await pipeline.process_crypto_news(
        symbols=symbols, hours_back=hours_back, enable_enrichment=enable_enrichment
    )
