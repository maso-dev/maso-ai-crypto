...
class ValidationResult(BaseModel):
    ...
    conflicting_sources: List[str] = Field(description="Sources that conflict with claims")
...
""")
    ...
    async def search_for_validation(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = tavily_client.search(
                ...
                include_domains=["coindesk.com", "cointelegraph.com", "reuters.com", "bloomberg.com", "wsj.com", "ft.com"],
    ...
    def _extract_claims(self, article: Dict[str, Any]) -> str:
        ...
        if any(word in title.lower() for word in ["price", "surge", "drop", "high", "low", "$"]):
            claims.append("price movements")
        
        # Extract regulatory claims
        if any(word in title.lower() for word in ["regulation", "sec", "ban", "approval", "legal"]):
            claims.append("regulatory developments")
        
        # Extract institutional claims
        if any(word in title.lower() for word in ["institutional", "etf", "blackrock", "fidelity", "adoption"]):
            claims.append("institutional adoption")
        
        # Extract technical claims
        if any(word in title.lower() for word in ["technology", "upgrade", "fork", "scaling"]):
...
---""")
    ...
    async def validate_articles_batch(self, articles: List[Dict[str, Any]]) -> List[ValidationResult]:
        """Validate multiple articles in batch."""
        print(f"🔍 Starting REACT validation for {len(articles)} articles...")
        
        validation_results = []
        for i, article in enumerate(articles, 1):
            print(f"  Validating article {i}/{len(articles)}: {article.get('title', '')[:50]}...")
            ...
            try:
                ...
                print(f"    ✅ Validation complete - Risk: {result.risk_level}, Confidence: {result.confidence_score:.2f}")
        ...
        print(f"🎉 REACT validation completed: {len(validation_results)} articles processed")
...
async def validate_news_articles(articles: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
