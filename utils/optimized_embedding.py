...
def create_summary_focused_chunk(
    article: Dict[str, Any], summary: str
) -> Dict[str, Any]:
    """
...
async def embed_summary_focused_chunk(chunk_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        resp = await client.embeddings.create(
            model="text-embedding-ada-002", input=chunk_data["searchable_text"]
        )

        chunk_data["dense_vector"] = resp.data[0].embedding

        # Create sparse vector from searchable text
        chunk_data["sparse_vector"] = compute_sparse_vectors(
...
async def process_article_for_optimized_storage(
    article: Dict[str, Any], enrichment_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    chunk_data = create_summary_focused_chunk(
        article, enrichment_data.get("summary", "")
    )
    ...
    graph_data = {
        ...
        "properties": {
            ...
            "temporal_relevance_score": chunk_data["metadata"][
...
def create_search_optimized_query(
