...
class IntelligentNewsCache:
    ...
    def _cache_query(self, cached_query: CachedNewsQuery):
        ...
        """, (
            ...
            cached_query.last_accessed.isoformat() if cached_query.last_accessed else None
        ))
    ...
    async def _update_personal_portfolio_tokens(self):
        ...
        try:
            ...
            if portfolio_data and portfolio_data.assets:
                ...
                print(f"📊 Updated personal portfolio tokens: {list(self.personal_portfolio_tokens.keys())}")
    ...
    async def get_portfolio_aware_news(
        ...
        if include_personal_portfolio:
            ...
            # Load from cache
            self.personal_portfolio_tokens = self._get_cached_portfolio_tokens("personal")
        ...
        for category_name, tokens in search_categories.items():
            ...
            for symbol, search_terms in tokens.items():
                ...
                if cached_result:
                    print(f"      ✅ Cache hit for {symbol} ({cached_result.hit_count} hits)")
                    total_cache_hits += 1
            ...
            if len(category_articles) > max_articles_per_category * len(tokens):
                category_articles = category_articles[:max_articles_per_category * len(tokens)]
        ...
        print(f"   📈 Cache hit rate: {(total_cache_hits / (total_cache_hits + total_cache_misses) * 100):.1f}%" if (total_cache_hits + total_cache_misses) > 0 else "N/A")
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        # Get expired queries
        cursor.execute("SELECT COUNT(*) FROM news_cache WHERE expires_at <= ?", 
                      (datetime.now(timezone.utc).isoformat(),))
        ...
        return {
            ...
            "average_hits_per_query": total_hits / total_queries if total_queries > 0 else 0,
            "popular_queries": [
                {
    ...
    def clear_expired_cache(self) -> int:
        ...
        cursor.execute("DELETE FROM news_cache WHERE expires_at <= ?", 
                      (datetime.now(timezone.utc).isoformat(),))
...
async def get_cached_news_for_symbols(symbols: List[str], hours_back: int = 24) -> List[Dict[str, Any]]:
