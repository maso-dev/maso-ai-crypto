...
# Local imports
from .vector_rag import EnhancedVectorRAG, VectorQuery, QueryType, intelligent_search, insert_enhanced_news_batch
...
class HybridRAGSystem:
    ...
    def __init__(self):
        ...
        print(f"   Graph RAG: {'✅' if self.graph_rag.connected else '❌ (using mock)'}")
    ...
    async def _react_hybrid_search(self, query: HybridQuery) -> List[HybridResult]:
        ...
        for result in react_results:
            hybrid_result = HybridResult(
                ...
                entity_mentions=[],
                confidence_score=result.similarity_score * 0.9  # Slightly lower for hybrid
            )
            hybrid_results.append(hybrid_result)
    ...
    async def _sentiment_analysis_search(self, query: HybridQuery) -> List[HybridResult]:
