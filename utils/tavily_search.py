...
class TavilySearchClient:
    ...
    async def search_news(self, query: str, max_results: int = 20, 
                         time_period: str = "1d") -> TavilySearchResponse:
        """
    ...
    async def search_finance(self, query: str, max_results: int = 15) -> TavilySearchResponse:
        """
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Tavily finance search failed: {response.status_code}")
                    return TavilySearchResponse(
                        query=query,
                        results=[],
                        total_results=0,
    ...
    async def search_web(self, query: str, max_results: int = 10) -> TavilySearchResponse:
        """
    ...
    def _parse_search_response(self, data: Dict[str, Any], search_type: str) -> TavilySearchResponse:
    ...
    async def get_crypto_news(self, symbols: List[str], max_results: int = 20) -> List[TavilySearchResult]:
        """
        ...
        for symbol in symbols:
            try:
                ...
                response = await self.search_news(query, max_results=max_results//len(symbols))
                
                if response.results:
                    all_results.extend(response.results)
                    logger.info(f"Found {len(response.results)} news articles for {symbol}")
                else:
        ...
        all_results.sort(key=lambda x: (x.score, x.published_date or datetime.min), reverse=True)
    ...
    async def get_trending_topics(self) -> List[str]:
        ...
        try:
            ...
            for result in response.results:
                ...
                if any(keyword in title for keyword in ["trending", "popular", "hot", "rising", "top"]):
...
async def search_crypto_news(symbols: List[str], max_results: int = 20) -> List[TavilySearchResult]:
...
async def search_tavily_finance(query: str, max_results: int = 15) -> TavilySearchResponse:
