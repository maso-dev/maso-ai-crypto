...
class CostTracker:
    ...
    def __init__(self, db_path: str = "cost_tracking.db"):
        ...
        self.cost_rates = {
            ...
            "tavily": {
                "search": {"per_call": 0.01}  # per search call
            },
            "newsapi": {
                "everything": {"per_call": 0.001}  # per API call
            },
            "milvus": {
    ...
    def calculate_cost(self, service: str, model: str, tokens_input: int = 0, tokens_output: int = 0) -> float:
    ...
    async def track_call(self, service: str, endpoint: str, model: str = "", 
    ...
    def get_daily_summary(self, date: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not daily_result:
            ...
            if result and result[0]:
                total_cost, call_count, openai_cost, tavily_cost, newsapi_cost, milvus_cost = result
                ...
                conn.commit()
                daily_result = (total_cost, openai_cost, tavily_cost, newsapi_cost, milvus_cost, call_count)
            else:
        ...
        total_cost, openai_cost, tavily_cost, newsapi_cost, milvus_cost, call_count = daily_result
    ...
    def get_monthly_summary(self, year: int, month: int) -> Dict[str, Any]:
        ...
        if result and result[0]:
            ...
            total_cost = call_count = openai_cost = tavily_cost = newsapi_cost = milvus_cost = 0
    ...
    def get_recent_calls(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        for row in cursor.fetchall():
            service, endpoint, timestamp, tokens_input, tokens_output, cost_usd, success, error_message = row
            calls.append({
                "service": service,
                "endpoint": endpoint,
                "timestamp": timestamp,
...
@asynccontextmanager
async def track_api_call(service: str, endpoint: str, model: str = "", 
                        tokens_input: int = 0, tokens_output: int = 0,
                        metadata: Optional[Dict[str, Any]] = None):
...
async def track_openai_call(model: str, tokens_input: int = 0, tokens_output: int = 0, 
                           metadata: Optional[Dict[str, Any]] = None):
...
async def track_tavily_call(endpoint: str = "search", metadata: Optional[Dict[str, Any]] = None):
...
async def track_newsapi_call(endpoint: str = "everything", metadata: Optional[Dict[str, Any]] = None):
    """Track NewsAPI call."""
    return await cost_tracker.track_call(
        service="newsapi",
        endpoint=endpoint,
