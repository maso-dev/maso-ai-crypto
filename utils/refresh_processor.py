...
class RefreshProcessor:
    ...
    async def run_refresh_processing(
        self, interval: RefreshInterval = RefreshInterval.DAILY
    ) -> ProcessingResult:
        """
    ...
    async def _run_full_refresh(self, result: ProcessingResult):
        ...
        try:
            ...
            result.processing_metadata["news_articles"] = len(
        ...
        if result.data_collected.get("news_data"):
            try:
                filtered_news = await self._filter_and_clean_data(
                    result.data_collected["news_data"]
                )
                result.data_collected["filtered_news"] = filtered_news
                result.processing_metadata["filtered_articles"] = len(
                    filtered_news.get("articles", [])
                )
            except Exception as e:
    ...
    async def _collect_price_data(self) -> Dict[str, Any]:
        ...
        for symbol in self.default_symbols:
            try:
                ...
                historical = await self.livecoinwatch.collect_historical_data(
                    symbol, days=30
                )
                if historical:
    ...
    async def _collect_news_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if news_result["success"] and news_result.get("articles"):
                ...
                logger.info(
                    f"✅ NewsAPI: Collected {len(news_result['articles'])} articles"
                )
            else:
                logger.warning(
                    f"⚠️ NewsAPI failed: {news_result.get('error', 'Unknown error')}"
                )
        ...
        try:
            if tavily_client and tavily_client.available:
                tavily_results = await tavily_client.get_crypto_news(
                    symbols=self.default_symbols, max_results=20
                )
        ...
        if all_articles:
            ...
            logger.info(
                f"✅ Combined {len(unique_articles)} unique articles from {sources_used}"
            )
            ...
            return {
                ...
                "metadata": {
                    "newsapi_articles": len(
                        [a for a in all_articles if "tavily_score" not in a]
                    ),
                    "tavily_articles": len(
                        [a for a in all_articles if "tavily_score" in a]
                    ),
    ...
    async def _calculate_basic_indicators(self) -> Dict[str, Any]:
        ...
        for symbol in self.default_symbols[:5]:  # Limit for quick refresh
            try:
                basic_indicators = (
                    await self.livecoinwatch.calculate_technical_indicators(
                        symbol, days=7
                    )
                )
                if basic_indicators:
        ...
        for symbol in self.default_symbols:
            try:
                technical_indicators = (
                    await self.livecoinwatch.calculate_technical_indicators(symbol)
                )
    ...
    async def _filter_and_clean_data(self, news_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            approved_articles = [
                fa.original_article for fa in filtered_articles if fa.is_approved
            ]
            rejected_articles = [
                fa.original_article for fa in filtered_articles if not fa.is_approved
            ]

            logger.info(
                f"✅ Filtered {len(approved_articles)}/{len(articles)} articles"
            )
            ...
            return {
                **news_data,
                ...
                "filtered_metadata": {
                    ...
                    "approval_rate": (
                        len(approved_articles) / len(articles) if articles else 0
                    ),
                },
            }
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.processing_stats,
            "success_rate": (
                self.processing_stats["successful_runs"]
                / self.processing_stats["total_runs"]
