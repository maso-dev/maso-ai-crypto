...
def calculate_temporal_relevance_score(
    published_at: str, current_time: Optional[datetime] = None
) -> Dict[str, float]:
    """
...
def sort_articles_by_temporal_relevance(
    articles: List[Dict], weights: Optional[Dict] = None
) -> List[Dict]:
    """
    ...
    def calculate_relevance_score(article: Dict) -> float:
        ...
        score = article["recency_score"] * weights.get("recency", 0.4) + article[
            "urgency_score"
        ] * weights.get("urgency", 0.6)
...
def get_temporal_context_summary(articles: List[Dict]) -> Dict:
    ...
    enhanced_articles = [
        enhance_article_with_temporal_context(article) for article in articles
    ]
    ...
    historical_count = sum(
        1 for a in enhanced_articles if a.get("is_historical", False)
    )

    avg_recency = sum(a.get("recency_score", 0) for a in enhanced_articles) / len(
        enhanced_articles
    )
    avg_urgency = sum(a.get("urgency_score", 0) for a in enhanced_articles) / len(
        enhanced_articles
    )
