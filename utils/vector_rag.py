...
from .milvus import MILVUS_URI, MILVUS_TOKEN, MILVUS_COLLECTION_NAME, insert_news_chunks, query_news_for_symbols
...
class EnhancedVectorRAG:
    ...
    async def _react_agent_search(
        ...
        if initial_results:
            ...
            print(f"✅ ReAct Agent: Combined {len(initial_results)} + {len(refined_results)} = {len(combined_results)} results")
    ...
    async def _hybrid_search(
        ...
        if query.symbols:
            ...
            keyword_vector_results = [self._parse_milvus_result(item) for item in keyword_results]
        else:
        ...
        # Combine results
        combined_results = self._combine_and_rank_results(semantic_results, keyword_vector_results)
        return combined_results[:query.limit]
    ...
    async def _temporal_search(
        ...
        metadata["temporal_filter"] = (datetime.now(timezone.utc) - timedelta(hours=query.time_range_hours)).isoformat()
